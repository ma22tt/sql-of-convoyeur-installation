...
**A partial unique index for "one load per machine", not a plain `UNIQUE`.** The invariant only concerns loads that are actually sitting somewhere. A plain unique index on `machine_id` would also compare the `NULL`s, and depending on the engine it either does nothing useful or blocks a second pallet from being in transit. `WHERE machine_id IS NOT NULL` states exactly which rows the rule applies to.
